function [Udegs,Vdegs,lonw,latw,Ucms,Vcms,u,v,lon,lat]=getncUV_aviso_cmems_product(path_to_product,nday,wrk_dir)
...
fw=find(lonw>180); % steph uncommented
lonw(fw)=lonw(fw)-360; % steph uncommented
[lonw,lonsrti]=sort(lonw); % steph uncommented
lonw=lonw([(nlon-1) nlon 1:nlon 1]); % steph uncommented
lonw(1:2)=lonw(1:2)-360; % steph uncommented
lonw(end)=lonw(end)+360; % steph uncommented
...
Ucms=Ucms(lonsrti,:) % steph uncommented
Vcms=Vcms(lonsrti,:);% steph uncommented
Ucms=Ucms([(nlon-1) nlon 1:nlon 1],:);% steph uncommented
Vcms=Vcms([(nlon-1) nlon 1:nlon 1],:);% steph uncommented

else %If the raccorded lonw has a break, then lon was regional: hence resetting lonw to lon % steph uncommented
lonw=lon;% steph uncommented
end % steph uncommented
